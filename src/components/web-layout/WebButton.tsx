...
import { LucideIcon, ShoppingCart, ShoppingBasket, Share2 } from 'lucide-react';
...
const WebButton: React.FC<WebButtonProps> = ({
  ...
  const baseClasses =
    'inline-flex items-center gap-2 font-medium rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zatobox-300';

  // To share the same design between buttons
  const BUTTON_VARIANTS = {
    HEADER_SHARE:
      'bg-transparent text-zatobox-500 hover:bg-zatobox-600 hover:text-white',
  ...
  const variantClasses = {
    ...
    outline:
      'bg-transparent border border-gray-300 text-gray-600 hover:bg-gray-100 focus:ring-gray-300',
  ...
  const isShoppingList =
    variant === 'shoppinglist' || variant === 'shoppinglist-active';
  const isShareProduct =
    variant === 'shareProduct' || variant === 'shareProduct-active';
  const isSpecialButton = isShoppingList /*|| isWishlist*/ || isShareProduct;

  const finalSizeClasses = isSpecialButton
    ? specialButtonSizes[size]
    : sizeClasses[size];
  ...
  let finalIcon = Icon;
  if (!Icon) {
    if (variant === 'buy' || variant === 'shoppingCar')
      finalIcon = ShoppingCart;
    // if (variant === 'wishlist' || variant === 'wishlist-active') finalIcon = Heart;
    if (variant === 'shoppinglist' || variant === 'shoppinglist-active')
      finalIcon = ShoppingBasket;
    if (
      variant === 'shareProduct' ||
      variant === 'shareProduct-active' ||
      variant === 'shareStoreHeader'
    )
      finalIcon = Share2;
  }
  ...
  return (
    <button
      ...
      className={`${finalBaseClasses} ${
        variantClasses[variant as keyof typeof variantClasses]
      } ${finalSizeClasses} ${disabledClasses} ${className} relative`}
    >
      {finalIcon &&
        React.createElement(finalIcon, {
          className: `w-4 h-4 ${
            /*variant === 'wishlist-active' ||*/ variant ===
              'shoppinglist-active' || variant === 'shareProduct-active'
              ? 'fill-current'
              : ''
          }`,
        })}
