...
# Cargar variables locales de desarrollo
_base_env = os.path.join(os.path.dirname(__file__), ".env")
_local_env = os.path.join(os.path.dirname(__file__), "env.local")

load_dotenv(_base_env, override=False)
load_dotenv(_local_env, override=True)

# Configurar la ruta de Tesseract de forma portable
# - En Docker/Linux normalmente tesseract está en PATH; no hace falta setear tesseract_cmd
# - Si necesitás forzar una ruta local (Windows), exportar TESSERACT_CMD o usar env.local
...
# Respetar TESSDATA_PREFIX si lo pasaron por env (Dockerfile ya lo setea en producción)
_tessdata = os.getenv("TESSDATA_PREFIX", "").strip()
if _tessdata:
...
def extract_all_products_ultra(full_text):
    ...
    while i < len(lines):
        ...
        if (
            ...
            or line.startswith("Invoice")
            or line.startswith("Payment")
            or line.startswith("Subtotal")
            or line.startswith("Tax")
            or line.startswith("Total:")
            or line.startswith("Name Description")
            or line in ["Description", "Quantity", "Unit Price", "Total", "Name"]
        ):
            i += 1
            continue
        ...
        if (
            ...
            while j < len(lines) and j < i + 5:  # Buscar en las próximas 4 líneas
                ...
                price_match = re.search(
                    r"(\d+)\s+\$(\d+(?:\.\d{2})?)\s+\$(\d+(?:\.\d{2})?)$", check_line
                )
                if price_match:
                ...
                # Si es una línea de descripción, agregarla
                    ...
                    len(check_line) > 10
                    and not re.match(r"^[A-Za-z]+$", check_line)
...
def process_invoice():
    ...
    try:
        ...
        if file.filename.lower().endswith(".pdf"):
            ...
            try:
                ...
                images = convert_from_bytes(
                    file_content, first_page=1, last_page=1, dpi=300
                )
                if images:
                    image = images[0]
        ...
        print(
            f"Metadatos extraídos: {len([v for v in result['metadata'].values() if v != 'No detectado'])}"
        )
...
if __name__ == "__main__":
    # En local ejecuta con debug off/on según FLASK_ENV
