...
export function PricingSection() {
  ...
  const pricingPlans = [
    ...
    },
    {
      ...
      monthlyPrice: "$0",
      annualPrice: "$0",
      description: getTranslation(language, "pricing.plans.starter.description"),
    ...
    },
    {
      ...
      monthlyPrice: "$0",
      annualPrice: "$0",
      description: getTranslation(language, "pricing.plans.enterprise.description"),
      features: getTranslation(language, "pricing.plans.enterprise.features"),
      buttonText: getTranslation(language, "pricing.plans.enterprise.buttonText"),
      buttonClass:
    ...
    <section className="w-full px-5 overflow-hidden flex flex-col justify-start items-center my-0 py-8 md:py-14">
      ...
      <div className="self-stretch px-5 flex flex-col md:flex-row justify-start items-start gap-4 md:gap-6 mt-6 max-w-[1100px] mx-auto">
        {pricingPlans.map((plan) => (
          <div
            key={plan.name}
            className={`flex-1 p-4 overflow-hidden rounded-xl flex flex-col justify-start items-start gap-6 h-full min-h-[600px] ${plan.popular ? "bg-primary shadow-[0px_4px_8px_-2px_rgba(0,0,0,0.10)]" : "bg-card"}`}
            ...
            <div className="self-stretch flex flex-col justify-between items-start gap-6 h-full">
              ...
              >
                <div className="px-1.5 flex justify-center items-center gap-2">
                  <span
                    className={`text-center text-sm font-medium leading-tight ${plan.name === getTranslation(language, "pricing.plans.tester.name") ? "text-gray-800" : plan.name === getTranslation(language, "pricing.plans.starter.name") ? "text-primary" : "text-zinc-950"}`}
                  >
                    {plan.buttonText}
                  </span>
                </div>
