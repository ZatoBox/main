...
const PrintableReceiptModal: React.FC<Props> = ({
  ...
  return (
    <ResponsiveModal
      ...
      title={t('receipts.printModal.title')}
      desktopClassName="max-w-2xl"
      footer={
        <div className="flex w-full flex-col sm:flex-row gap-3">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2.5 min-h-11 text-sm font-medium rounded-lg transition-all duration-200 bg-white text-[#6B7280] border border-[#E5E7EB] hover:bg-[#F9FAFB]"
          >
            {t('receipts.printModal.close')}
          </button>
          <button
            onClick={handleDownloadPDF}
            className="flex-1 flex items-center justify-center space-x-2 px-4 py-2.5 min-h-11 text-sm font-medium rounded-lg transition-all duration-200 bg-white text-[#6B7280] border border-[#E5E7EB] hover:bg-[#F9FAFB]"
          >
            <Download size={16} />
          ...
          <button
            onClick={handlePrint}
            className="flex-1 flex items-center justify-center space-x-2 px-4 py-2.5 min-h-11 text-sm font-medium rounded-lg transition-all duration-200 bg-[#F88612] text-white hover:bg-[#E07A0A] shadow-sm hover:shadow-md"
          >
            <Printer size={16} />
    ...
    >
      <div
        ref={printRef}
        className="bg-white p-4 sm:p-8 border border-[#E5E7EB] rounded-lg mb-2"
        style={{ fontFamily: 'Arial, sans-serif' }}
      >
        <div className="text-center mb-8 pb-6 border-b-2 border-[#E5E7EB]">
          <h1 className="text-3xl font-bold text-[#F88612] mb-2">ZatoBox</h1>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 text-sm">
          <div>
            <p className="text-[#9CA3AF]">{t('receipts.printModal.date')}</p>
            <p className="font-semibold text-[#000000]">{date}</p>
          </div>
          <div className="text-right">
            <p className="text-[#9CA3AF]">{t('receipts.printModal.status')}</p>
