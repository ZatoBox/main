...
export async function POST(req: NextRequest) {
  try {
    const { userId, items, metadata, ownerId } = await req.json();
    ...
    const cashProduct = await polarAPI.createProduct(polarApiKey, {
      name: `Cash Order #${Math.floor(Math.random() * 1000000)}`,
    ...
    const checkoutData = {
      ...
      metadata: {
        user_id: finalUserId,
        cart_product_id: cashProduct.id,
        payment_type: 'cash',
        ...metadata,
      },
    };
