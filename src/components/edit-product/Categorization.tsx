  ...
  return (
    <div className="p-6 border rounded-lg shadow-sm bg-white border-[#CBD5E1] space-y-4">
      <h3 className="text-lg font-medium text-black">
        {t('editProduct.categorization.title')}
      </h3>
      {loading ? (
        <div className="text-sm text-gray-500">
          {t('editProduct.categorization.loading')}
        </div>
      ) : (
        <>
          <input
            type="text"
            placeholder={t('editProduct.categorization.searchPlaceholder')}
            value={query}
          ...
          <div className="max-h-48 overflow-auto pr-1 space-y-1 custom-scrollbar">
            ...
            {filtered.length === 0 && (
              <div className="px-2 py-4 text-xs text-center text-gray-400">
                {t('editProduct.categorization.noMatches')}
              </div>
            )}
          </div>
